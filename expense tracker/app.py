...
def create_app(test_config=None):
    ...
    def add_expense():
        ...
        new_expense = Expense(amount=amount, category=category, date=date_val, note=note)
    ...
    def edit_expense(expense_id):
        ...
        category = db.session.execute(select(Category).where(Category.id == category_id)).scalars().first()
        if not category:
